...
public class Parser {
	...
	public Parser() {
		List<List<String>> lex_rules =tripleo.elijah_fluffy.util.Helpers.List_of(
				List_of("if", "IF"),
				List_of("then", "THEN"),
				List_of("else", "ELSE"),
				List_of("true", "TRUE"),
...
//		}
	...
	Decl_AST _decl() throws ParseError {
		...
		List<String> argnames =tripleo.elijah_fluffy.util.Helpers.List_of();
	...
	AppExpr_AST _app(String name) throws ParseError {
		this._match("(");
		List<AstNode> args =tripleo.elijah_fluffy.util.Helpers.List_of();
