...
//		super(name);
	...
	void main2_el(final CompilerContext cctx, final @NotNull GenBuffer gbn) {
		...
		final LocalDeclAgnNode ldan_f1 = new LocalDeclAgnNode("f1",
				convertToLocalAgnTmpNode2(ExpressionNodeBuilder.fncall("factorial", tripleo.elijah_fluffy.util.Helpers.List_of(new LocalAgnTmpNode("b1")))));
		gbn.GenLocalAgn(cctx, ldan_f1);
		
		final SimpleFnCall sfc = new SimpleFnCall("print",tripleo.elijah_fluffy.util.Helpers.List_of("f1"));
	...
	private List<LocalAgnTmpNode> convertToLocalAgnTmpNode(final List<@NotNull NumericExpression>tripleo.elijah_fluffy.util.Helpers.List_of) {
...
//		GenReturnAgnSimpleInt(cctx, rasin, gbn);
		...
		final LocalAgnTmpNode latn2=new LocalAgnTmpNode(tccssan2, ExpressionNodeBuilder.fncall(
				"factorial_r",tripleo.elijah_fluffy.util.Helpers.List_of(lamn)));
