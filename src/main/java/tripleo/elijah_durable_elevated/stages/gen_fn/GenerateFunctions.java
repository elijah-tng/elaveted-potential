...
import tripleo.elijah_durable_elevated.stages.gen_fn_c.*;
...
public class GenerateFunctions implements ReactiveDimension, GGenerateFunctions {
	...
	public GenerateFunctions(final OS_Module aModule, final GenFnC aBc) {
		...
		LOG = new EDL_ElLog(module.getFileName(), phase.getVerbosity(), PHASE);
	...
	private void assign_variable(final @NotNull BaseEvaFunction gf, final int vte, @NotNull final IExpression value,
		...
		case PROCEDURE_CALL:
			...
			final int ii2 = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(vte, gf), fnCallArgs), cctx);
		...
		case NUMERIC:
			...
			final int ii = add_i(gf, InstructionName.AGNK,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(vte, gf), new ConstTableIA(ci, gf)),
					cctx);
		...
		case IDENT:
			...
			final int ii3 = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(vte, gf), ia1), cctx);
	...
	private int add_i(@NotNull final BaseEvaFunction gf, final InstructionName x,
					  final List<InstructionArgument> l, final Context ctx) {
		final int i = gf.add(x,l, ctx);
		return i;
	}
...
//			LOG.err("7056 aConstructorDef.getItem = "+item);
		...
		final int x1 = add_i(gf, InstructionName.X,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(e1, gf)), cctx);
...
//				LOG.err("109 "+expression);
	...
	public InstructionArgument simplify_expression(@NotNull final IExpression expression, final @NotNull BaseEvaFunction gf,
		...
		case CAST_TO: {
			...
			final int x = add_i(gf, InstructionName.CAST_TO,tripleo.elijah_fluffy.util.Helpers.List_of(simp, new IntegerIA(tte_index.getIndex(), gf)),
					cctx);
		}
		case AS_CAST: {
			@NotNull
			...
			TypeTableEntry tte_index = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT,
					new OS_UserType(tce.getTypeName()));
			final int x = add_i(gf, InstructionName.AS_CAST,tripleo.elijah_fluffy.util.Helpers.List_of(simp, new IntegerIA(tte_index.getIndex(), gf)),
					cctx);
		}
		case DOT_EXP: {
		...
		case GET_ITEM: {
			...
			}
			{
				...
				final @NotNull TypeTableEntry tte_right = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, null,
						right);
				final int pte = addProcTableEntry(expr_kind_name, null,tripleo.elijah_fluffy.util.Helpers.List_of(tte_left, tte_right), gf);
				final int tmp = addTempTableEntry(new OS_BuiltinType(BuiltInTypes.Boolean), gf); // README should be
																									// Boolean
				add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(tmp, gf)), cctx);
				...
				final int x = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(tmp, gf), fca), cctx);
...
//					TypeTableEntry tte = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, null, expr_kind_name);
				final @NotNull TypeTableEntry tte_left = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, null,
						left);
				final int pte = addProcTableEntry(expr_kind_name, null,tripleo.elijah_fluffy.util.Helpers.List_of(tte_left), gf);
				final int tmp = addTempTableEntry(new OS_BuiltinType(BuiltInTypes.Boolean), gf);
				add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(tmp, gf)), cctx);
				final @NotNull Instruction inst = new Instruction();
				inst.setName(InstructionName.CALLS);
...
//			break;
		case ASSIGNMENT: {
			...
			final int x = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(s1, s2), cctx);
...
//						throw new IllegalStateException("Cant be here");
		...
		right_ia.add(0, new ProcIA(pte, gf));
		{
			...
			add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(tmp_var, gf)), cctx);
			final @NotNull Instruction i = new Instruction();
			i.setName(InstructionName.CALL);
			i.setArgs(right_ia);
			// TODO should be AGNC
			final int x = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(tmp_var, gf), new FnCallArgs(i, gf)),
					cctx);
	...
	static class GIA__procedure_call__one {
		...
		private @NotNull List<InstructionArgument> list_of_fn_call =new ArrayList<>();
		...
		public GIA__procedure_call__one(final @NotNull BasicBinaryExpression bbe,
			...
			list_of_fn_call =tripleo.elijah_fluffy.util.Helpers.List_of(lookup, new FnCallArgs(expression_to_call, gf));
		}
		...
		private void action__IdentExpression(final StatementWrapper aStatementWrapper) {
			...
			add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(vte_num, gf)), cctx);
			// TODO should be AGNC
			final List<InstructionArgument> list_of_fn_call2 =tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(vte_num, gf),
...
//			klass.attached.setCode(nextClassCode());
		}
	}
...
//				LOG.info("711 " + i);
				...
				add_i(gf, InstructionName.JNE,tripleo.elijah_fluffy.util.Helpers.List_of(i, new ConstTableIA(const_true, gf), label_next), cctx);
				...
				add_i(gf, InstructionName.XS,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(begin_2nd, gf)), cctx);
				if (ifc.getParts().size() == 0) {
					gf.place(label_next);
					add_i(gf, InstructionName.XS,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(begin_1st, gf)), cctx);
//					gf.place(label_end);
				} else {
					add_i(gf, InstructionName.JMP,tripleo.elijah_fluffy.util.Helpers.List_of(label_end), cctx);
...
//						label_next = gf.addLabel();
						if (part.getExpr() != null) {
							...
							LOG.info("712 " + ii);
							add_i(gf, InstructionName.JNE,tripleo.elijah_fluffy.util.Helpers.List_of(ii, new ConstTableIA(const_true, gf), label_next),
								  cctx);
						}
						...
						add_i(gf, InstructionName.XS,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(begin_next, gf)), cctx);
						gf.place(label_next);
					}
					gf.place(label_end);
				}
				add_i(gf, InstructionName.XS,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(begin0, gf)), cctx);
			}
		}
...
//			LOG.info("702 "+loop.getType());
			...
			final int x2 = add_i(gf, InstructionName.XS,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(e2, gf)), cctx);
		...
		private void generate_loop_EXPR_TYPE(@NotNull Loop loop, @NotNull BaseEvaFunction gf, @NotNull Context cctx) {
			...
			add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(loop_iterator, gf)), cctx);
...
//			if (ia1 instanceof ConstTableIA)
			add_i(gf, InstructionName.AGNK,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(loop_iterator, gf), ia1), cctx);
//			else
//				add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(loop_iterator), ia1), cctx);
			...
			add_i(gf, InstructionName.JE,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(loop_iterator, gf),
			...
			final int pre_inc = addProcTableEntry(pre_inc_name, null,tripleo.elijah_fluffy.util.Helpers.List_of(tte), gf);
			add_i(gf, InstructionName.CALLS,tripleo.elijah_fluffy.util.Helpers.List_of(new ProcIA(pre_inc, gf), new IntegerIA(loop_iterator, gf)), cctx);
			add_i(gf, InstructionName.JMP,tripleo.elijah_fluffy.util.Helpers.List_of(label_top), cctx);
		...
		private void generate_loop_FROM_TO_TYPE(@NotNull Loop loop, @NotNull BaseEvaFunction gf,
			...
			add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(iter_temp, gf)), cctx);
			final InstructionArgument ia1 = simplify_expression(loop.getFromPart(), gf, cctx);
			if (ia1 instanceof ConstTableIA)
				add_i(gf, InstructionName.AGNK,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(iter_temp, gf), ia1), cctx);
			else
				add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(iter_temp, gf), ia1), cctx);
			final @NotNull Label label_top = gf.addLabel("top", true);
			gf.place(label_top);
			final @NotNull Label label_bottom = gf.addLabel("bottom" + label_top.getIndex(), false);
			add_i(gf, InstructionName.JE,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(iter_temp, gf),
												  simplify_expression(loop.getToPart(), gf, cctx), label_bottom), cctx);
			for (final StatementItem statementItem : loop.getItems()) {
			...
			final @NotNull TypeTableEntry tte = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, null, pre_inc_name);
			final int pre_inc = addProcTableEntry(pre_inc_name, null,tripleo.elijah_fluffy.util.Helpers.List_of(tte/* getType(left), getType(right) */),
												  gf);
			add_i(gf, InstructionName.CALLS,tripleo.elijah_fluffy.util.Helpers.List_of(new ProcIA(pre_inc, gf), new IntegerIA(iter_temp, gf)), cctx);
			add_i(gf, InstructionName.JMP,tripleo.elijah_fluffy.util.Helpers.List_of(label_top), cctx);
			gf.place(label_bottom);
		}
					...
					for (final MC1 part : mc.getParts()) {
						if (part instanceof final @NotNull MatchArm_TypeMatch mc1) {
							...
							add_i(gf, InstructionName.IS_A,
								 tripleo.elijah_fluffy.util.Helpers.List_of(i, new IntegerIA(t.getIndex(), gf), /* TODO not */new LabelIA(label_next)),
								  cctx);
							final Context context = mc1.getContext();

							add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(tmp, gf)),
								  context);
							...
							add_i(gf, InstructionName.JMP,tripleo.elijah_fluffy.util.Helpers.List_of(label_end), context);
							add_i(gf, InstructionName.XS,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(begin0, gf)), cctx);
							gf.place(label_next);
							label_next = gf.addLabel();
							...
							add_i(gf, InstructionName.JNE,tripleo.elijah_fluffy.util.Helpers.List_of(i, i2, label_next), cctx);
...
//							label_next = gf.addLabel();
					...
					gf.place(label_next);
					add_i(gf, InstructionName.NOP,tripleo.elijah_fluffy.util.Helpers.List_of(), cctx);
					gf.place(label_end);
				}
			}
		}
...
//						final IExpression right1 = bbe.getRight();
					...
					final @NotNull IdentExpression fn_aug_name = Helpers0
							.string_to_ident(SpecialFunctions.of(expressionKind));
					final @NotNull List<TypeTableEntry> argument_types =tripleo.elijah_fluffy.util.Helpers.List_of(
...
//						LOG.info("801.2 "+argument_types);
					...
					final int i = add_i(gf, InstructionName.CALLS,tripleo.elijah_fluffy.util.Helpers.List_of(new ProcIA(fn_aug, gf), left, right), cctx);
					//
...
//				LOG.info(String.format("8004-2 %s %s;", stype, vs.getName()));
				...
				case 1: {
					...
					add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("const"), new IntegerIA(vte_num, gf)), cctx);
					add_i(gf, InstructionName.AGNK,tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(vte_num, gf), new ConstTableIA(ci, gf)),
						  cctx);
					break;
				}
				case 2: {
					...
					add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("val"), new IntegerIA(vte_num, gf)), cctx);
					final @NotNull IExpression iv = initialValue;
				...
				case 3: {
					...
					// vs.initialValue ??
					add_i(gf, InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("var"), new IntegerIA(vte_num, gf)), cctx);
					final @NotNull IExpression iv = initialValue;
					assign_variable(gf, vte_num, iv, cctx);
					break;
				}
	...
	class Generate_item_assignment {
		...
		public void dot(@NotNull BaseEvaFunction gf, @NotNull DotExpression left, @NotNull IExpression right,
			 ...
			 * add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(new IdentIA(ident_lright, gf), new
			 * IdentIA(ident_right, gf)), cctx);
			 */

			final int inst2 = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(simple_left, simple_right), cctx);
		...
		public void ident(@NotNull BaseEvaFunction gf, @NotNull IdentExpression left, @NotNull IdentExpression right,
			...
			if (vte_right == null) {
				...
				inst = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(some_left, new IdentIA(ident_right, gf)), cctx);
			} else {
				inst = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(some_left, vte_right), cctx);
...
//					((IdentIA) some_left).getEntry().addPotentialType(inst, unknown_type);
		...
		public void mathematical(@NotNull BaseEvaFunction gf, @NotNull IExpression left, ExpressionKind kind,
			...
			final int instruction_number = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(left_ia, right_ia), cctx);
			final Instruction instruction = gf.getInstruction(instruction_number);
...
//				idte.type = tte;
		...
		public void numeric(@NotNull BaseEvaFunction gf, @NotNull IExpression left, @NotNull NumericExpression ne,
			...
			final int agn_inst = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(agn_path, new ConstTableIA(cte, gf)), cctx);
			// TODO what now??
		}
		...
		public void procedure_call(final StatementWrapper aStatementWrapper, @NotNull BaseEvaFunction gf,
			 ...
			 * final List<InstructionArgument>list_of_fn_call =tripleo.elijah_fluffy.util.Helpers.List_of(lookup, new
			 ...
			 * InstructionName.DECL,tripleo.elijah_fluffy.util.Helpers.List_of(new SymbolIA("tmp"), new IntegerIA(vte_num,
			 * gf)), cctx); // TODO should be AGNC final List<InstructionArgument>
			 *list_of_fn_call2 =tripleo.elijah_fluffy.util.Helpers.List_of(new IntegerIA(vte_num, gf), new
		...
		public void string_literal(@NotNull BaseEvaFunction gf, @NotNull IExpression left, StringExpression right,
			...
			final int agn_inst = add_i(gf, InstructionName.AGN,tripleo.elijah_fluffy.util.Helpers.List_of(agn_path, new ConstTableIA(cte, gf)), aContext);
			// TODO what now??
		}
	}
