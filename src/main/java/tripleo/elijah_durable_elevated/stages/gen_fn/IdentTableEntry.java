...
import tripleo.elijah_durable_elevated.stages.deduce.Resolve_Ident_IA.*;
...
public class IdentTableEntry extends BaseTableEntry1
	...
	private final          DeferredObject<InstructionArgument, Void, Void> _p_backlinkSet             = new DeferredObject<>();
	private final          DeferredObject<ProcTableEntry, Void, Void>      _p_constructableDeferred   = new DeferredObject<>();
	private final          DeferredObject<GenType, Void, Void>             _p_fefiDone                = new DeferredObject<>();
	...
	public IdentTableEntry(final int index, final IdentExpression ident, Context context,
		...
		typeResolve.typeResolution().then(gt -> {
			if (type != null && type.getGenType() != null) // !! 07/30
				type.getGenType().copy(gt);
		});
	}
	...
	public @NotNull DeducePath buildDeducePath(BaseEvaFunction generatedFunction) {
		// README 24/08/08 we can see how old this is
		final IdentIA                      identIA                 = new IdentIA(index, generatedFunction);
		@NotNull List<InstructionArgument> instructionArgumentList = StaticEva._getIdentIAPathList(identIA);
		return new DeducePath(this, instructionArgumentList);
	}
	...
	public void calculateResolvedElement() {
		...
		if (resolved_element != null) {
			...
			// 08/13 tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("283283 resolution failure for " +
	...
	public class _Reactive_IDTE extends DefaultReactive {
		...
		public <IdentTableEntry> void addResolveListener(final @NotNull Consumer<IdentTableEntry> t) {
			_p_resolveListenersPromise
					.then((DoneCallback<? super tripleo.elijah_durable_elevated.stages.gen_fn.IdentTableEntry>) result -> t
