...
//		expressionConfession = ExpressionConfession.from(expression, expression_num);
	...
	public String getLoggingString(final @Nullable DeduceTypes2 aDeduceTypes2) {
		...
		TypeTableEntry typeTableEntry : getArgs()) {
			...
			if (attached != null)
				...
				final IExpression debugExpression = typeTableEntry.get__debug_expression();
				if (debugExpression != null)
					l.add(String.format("<Unknown expression: %s>", debugExpression));
				else
					l.add("<Unknkown>");
			}
		}
	...
	public void onSetAttached() {
		...
		case 1:
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("136 pte not finished resolving " + this);
			break;
		case 2:
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("138 Internal compiler error");
			break;
		case 3:
