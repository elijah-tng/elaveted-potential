...
public class EvaClass extends EvaContainerNC implements GNCoded, GEvaClass {
	...
	private       ElevatedLivingClass _living;
			...
			varTableEntry.updatePotentialTypesCB = new VarTableEntry.UpdatePotentialTypesCB() {
				...
				public @NotNull Operation<Boolean> call(final @NotNull EvaContainer aEvaContainer) {
					...
					if (potentialTypes.size() == 2) {
						...
						if (potentialTypes.get(1).getResolved() instanceof final OS_BuiltinType resolved
						...
						} else if (potentialTypes.get(0).getResolved() instanceof final OS_BuiltinType resolved
					...
					for (TypeTableEntry potentialType : varTableEntry.potentialTypes) {
						...
						try {
							if (potentialType.getGenType().getTypeName() == null) {
								...
								if (potentialType.getGenType().getResolved() == null
										&& potentialType.getGenType().getResolvedn() == null) {
...
//					final Set<GenType> s = Collections.unmodifiableSet(set);
	...
	public ElevatedLivingClass getLiving() {
		return world();
	}
	...
	public void setLiving(ElevatedLivingClass _living) {
	...
	public ElevatedLivingClass world() {
		return _living;
	}
}
