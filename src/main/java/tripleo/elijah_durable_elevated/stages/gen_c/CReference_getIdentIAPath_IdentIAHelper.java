...
class CReference_getIdentIAPath_IdentIAHelper {
	...
	private boolean _act_ClassStatement(final @NotNull CReference aCReference, boolean b) {
		...
		if (getResolved() != null) {
			...
			code = -1;
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err("** 31116 not resolved " + getResolved_element());
		}
	...
	private void _act_ConstructorDef(final @NotNull CReference aCReference) {
		...
		if (getResolved() != null) {
			...
			code = -1;
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err("** 31161 not resolved " + getResolved_element());
		}
		// README Assuming this is for named constructors
	...
	private void _act_DefFunctionDef(final @NotNull CReference aCReference) {
		...
		getSl().clear();
		if (code == -1) {
//				text2 = String.format("ZT%d_%d", enclosing_function.attached.getCode(), closure_index);
		}
				...
				resolvedFunction.onGenClass((EvaClass gc) -> {
					...
					if (gc instanceof EvaContainerNC) // and not another function
					{
						...
						if (this.code == 0){
							...
							final Compilation      compilation0 = gc.getElement().getContext().module().getCompilation();
							final EDL_ICompilation compilation  = (EDL_ICompilation) compilation0;
...
//							gc.setCode();
	...
	boolean action(final CRI_Ident aCRI_ident, final @NotNull CReference aCReference) {
		...
		if (resolvedElement instanceof ClassStatement) {
			...
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out("1008 " + resolvedElement.getClass().getName());
