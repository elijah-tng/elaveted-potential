...
import static tripleo.elijah_durable_elevated.util.Helpers0.String_join;
...
//		return Helpers.String_join("->", sl);
	...
	public void debugPath(final @NotNull IdentIA identIA, final @NotNull String aPath) {
		...
		if (DebugFlags.FORCE_IGNORE) {
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4("\\ 172-172-172-172-172 ---------------------------------------------");
			for (InstructionArgument instructionArgument : pl) {
				if (instructionArgument instanceof final @NotNull ProcIA procIA) {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4(""+procIA.getEntry().__debug_expression);
				} else if (instructionArgument instanceof final @NotNull IdentIA argument) {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4(argument.getEntry().getIdent().getText());
				} else if (instructionArgument instanceof final @NotNull IntegerIA integerIA) {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4(integerIA.getEntry().getName());
				}
			}
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4("- 172-172-172-172-172 ---------------------------------------------");
	...
	private String getIdentIAPath__IdentIA(final @NotNull IdentIA ia2, final Generate_Code_For_Method.@NotNull AOG aog,
		...
		// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("181181 " + text + " " + item.getText());
		// assert text != null;
		if (text == null)
			return "<<null 181>>";
		return text;
