	...
	public @Nullable String getIdentIAPath(int i, final int sSize, final Generate_Code_For_Method.@NotNull AOG aog,
		...
		if (repo_element != null) {
			...
			if (!skip[0]) {
				...
				case 1:
					if (resolved == null) {
						tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err("***88*** resolved is null for " + ite);
					}
					if (sSize >= i + 1) {
...
//						assert false;
				...
				tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err("119 " + ite.getIdent() + " " + ite.getStatus());
