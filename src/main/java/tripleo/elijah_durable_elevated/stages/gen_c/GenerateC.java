...
public class GenerateC implements CodeGenerator, GenerateFiles, ReactiveDimension {
	...
	public GenerateC(final @NotNull OutputFileFactoryParams aParams,
		...
		final EDL_ElLog.Verbosity verbosity = aParams.getVerbosity();

		LOG = new EDL_ElLog(mod.getFileName(), verbosity, PHASE);
	...
	public void generate_class(@NotNull GenerateResultEnv aFileGen, EvaClass x) {
		...
		final ElevatedLivingClass lc = aResultSink.getLivingClassForEva(x); // TODO could also add _living property
...
//				VariableTableEntry vte = gf.getVarTableEntry(DeduceTypes2.to_int(ia));
	...
	public CReference new_CReference() {
		//noinspection UnnecessaryLocalVariable
		final CReference res = new CReference(_repo, _ce());
		return res;
	}
	...
	enum GetTypeName {
		...
		static String forOSType(final @NotNull OS_Type ty, @NotNull ElLog LOG) {
			...
			case USER_CLASS:
				...
				z = Emit.emit("/*443*/") + String.format("Z%d/*%s*/", -4 /*el.attached.getCode()*/, name);
				break;
			case FUNCTION:
				z = "<function>";
				break;
...
//			return getTypeName(new OS_UserType(typeName));
		...
		static String forVTE(@NotNull VariableTableEntry input) {
			...
			// special case
			//
			if (input.getTypeTableEntry().getGenType().getNode() != null)
				return Emit.emit("/*395*/") + getTypeNameForEvaNode(input.getTypeTableEntry().getGenType().getNode()) + "*";
			//
...
//			ClassInvocation ci = pte.getClassInvocation();
	...
	public GenerateResult resultsFromNodes(final @NotNull List<EvaNode> aNodes,
		...
		if (fg.resultSink() != grs) {
			// LOOK 09/28 eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
