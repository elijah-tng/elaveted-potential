...
public class Resolve_Ident_IA {
	...
	public void action() {
		dei.resolvedElementPromise().then((el2) -> {
			...
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_2("  70 " + el2);
...
//				assert idte.getStatus() != BaseTableEntry.Status.UNCHECKED;
			...
			if (idte.resolveExpectation == null) {
				tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("385 idte.resolveExpectation is null for " + idte);
			} else
...
//					prte.setResolvedElement(el);
	...
	private void action_002_no_resolved_element(final @NotNull ProcTableEntry pte, final @NotNull IdentTableEntry ite) {
		...
		if (ite.getBacklink() instanceof final @NotNull ProcIA backlink_) {
			@NotNull
		...
		} else if (ite.getBacklink() instanceof final @NotNull IntegerIA backlink_) {
			@NotNull
		...
		if (pte.getFunctionInvocation() == null) {
			...
			if (setClassInvocation) {
				if (ci != null) {
					...
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("542 Null ClassInvocation");
			}
	...
	private void preUpdateStatus(final @NotNull List<InstructionArgument> s) {
		...
		final var normal_path1 = StaticEva._getIdentIAResolvableList(identIA);
		final String normal_path  = generatedFunction.getIdentIAPathNormal(identIA);
		if (s.size() > 1) {
...
//		fi.setClassInvocation(ci);
	...
	public static class DeduceElementIdent implements IDeduceElement_old {
		...
		private final Eventual<OS_Element>                     _p_Element              = new Eventual<>();
		private       Context                                  context;
		private       BaseEvaFunction                          generatedFunction;
		private       DeduceTypes2                             deduceTypes2;

		{
			resolvedElementPromise().then(new DoneCallback<OS_Element>() {
				@Override
				public void onDone(final OS_Element result) {
					_p_Element.resolve(result);
				}
			});
		...
		public OS_Element getResolvedElement() {
			if (deduceTypes2 == null) { // TODO remove this ASAP. Should never happen
				tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("5454 Should never happen. gf is not deduced.");
				return null;
...
//			assert is_set[0] == true;
		...
		public void onElement(final DoneCallback<? super OS_Element> p) {
			_p_Element.then(p);
		}
		...
		public String getIdentIAPathNormal(final BaseEvaFunction aGeneratedFunction) {
			assert aGeneratedFunction == generatedFunction;
			return generatedFunction.getIdentIAPathNormal(new IdentIA(this.identTableEntry.getIndex(), generatedFunction));
		}
	}
	...
	class StatusListener__RIA__176 implements BaseTableEntry.StatusListener {
		...
		public StatusListener__RIA__176(final @NotNull IdentTableEntry aY, final FoundElement aFoundElement) {
			...
			normal_path1 = StaticEva._getIdentIAResolvableList(identIA);
