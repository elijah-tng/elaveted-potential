...
public class DeduceLocalVariable {
	...
	private void __pt_work_002(final @NotNull VariableTableEntry vte, final @NotNull ProcTableEntry procTableEntry,
		...
		if (resolvedElement instanceof BaseFunctionDef) {
			dm = deduceTypes2.deferred_member_function(null, (BaseFunctionDef) resolvedElement,
													   procTableEntry.getFunctionInvocation(), Self);
			pdm.resolve(dm);
		} else {
			procTableEntry.onFunctionInvocation(fi -> {
				if (fi.fd instanceof ConstructorDef cd) {
					final DeferredMemberFunction dm2 = deduceTypes2.deferred_member_function(null, cd,
																							 procTableEntry.getFunctionInvocation(), Self);

					pdm.resolve(dm2);
				} else {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("********************************* not a Constructor");
				}
			});
		}
	}
	...
	private void __pt_work_002b(final @NotNull VariableTableEntry vte, final @NotNull ProcTableEntry procTableEntry,
		...
		if (Self == null) {
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("336 ======================================================");
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("336 ======================================================");
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("336 ======================================================");
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("336 ======================================================");
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("336 ======================================================");
		...
		final @Nullable DeferredMemberFunction dm = deduceTypes2.deferred_member_function(null,
																						  (BaseFunctionDef) resolvedElement0, procTableEntry.getFunctionInvocation(), Self);
	...
	public void resolve_var_table_entry_for_exit_function() {
		...
		if (vte.getVtt() == VariableTableType.TEMP) {
			final GenType genType = vte.getTypeTableEntry().getGenType();
...
//					aResolveError.printStackTrace();
			...
			return;
		{
			if (vte.getTypeTableEntry().getAttached() == null && vte.getConstructable_pte() != null) {
				...
				vte.getTypeTableEntry().getGenType().setResolved(attached);
			...
			if (vte.getTypeTableEntry().getAttached() == null && vte.potentialTypes().size() > 0) {
				...
				if (attached_list.size() == 1) {
					...
					vte.getTypeTableEntry().getGenType().genCI(null, deduceTypes2, deduceTypes2._errSink(), deduceTypes2.phase);
					final @Nullable ClassInvocation classInvocation = (ClassInvocation) vte.getTypeTableEntry().getGenType().getCi();
					if (classInvocation != null) {
						classInvocation.resolvePromise().then(new DoneCallback<EvaClass>() {
							@Override
							public void onDone(final @NotNull EvaClass result) {
								vte.resolveTypeToClass(result);
								vte.getGenType().copy(vte.getTypeTableEntry().getGenType()); // TODO who knows if this is necessary?
							}
						});
					}
				} else {
...
//							type.resolve(_inj().new_GenType() {target.prototype}): // DeduceType??
	...
	public void resolve_var_table_entry_potential_types_1(final @NotNull VariableTableEntry vte,
		...
		if (vte.potentialTypes().size() == 1) {
			...
			if (tte1.getTableEntry() instanceof final @NotNull ProcTableEntry procTableEntry) {
				 ...
				 * 
				 * tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("410 " + t);
				 */
