...
public class Implement_construct {
	...
	public void action_IdentIA(final Context aContext) {
		...
		final Promise<DeduceElement, FCA_Stop, Void> p2 = dpc.targetP2();
		assert p2 != null;
		p2.then((final @NotNull DeduceElement target) -> {
			DeclAnchor xxv = target.declAnchor();
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4("144 " + xxv);
	...
	private void action_IdentIA___0001(final @NotNull IdentTableEntry idte, final @NotNull DeducePath deducePath,
		...
		for (int i = 0; i < deducePath.size(); i++) {
			...
			if (ia2 instanceof IntegerIA) {
				...
				assert el3 != null;
				assert i == 0;
				ectx[0] = deducePath.getContext(i);
				...
				assert ectx[0] != null;
				LookupResultList lrl = ectx[0].lookup(s);

				//if (el3 == null) {
				//int yy = 2;
				//}

				if (lrl == null && ectx[0] instanceof DeducePath.MemberContext) {
					p = (action_IdentIA___0001_5(deducePath, (DeducePath.MemberContext) ectx[0], i, idte2, s));
				} else {
...
//									aResolveError.printStackTrace();
	...
	private void action_IdentIA___0001_2(final @NotNull DeducePath deducePath, final int i,
		...
		if (type.getNonGenericTypeName() == null) {
			// HACK. not guaranteed to work!
			type.setNonGenericTypeName(Objects.requireNonNull(deducePath.getType(i - 1)).getNonGenericTypeName());
		}
		@NotNull
	...
	private void action_IdentIA___0001_1(final @NotNull IdentTableEntry idte, final @NotNull DeducePath deducePath,
		...
		if (type.getNonGenericTypeName() != null) {
			if (((NormalTypeName) type.getNonGenericTypeName()).getGenericPart().size() > 0) {
				// goal: create an equivalent (Regular)TypeName without the genericPart
			...
			// HACK. not guaranteed to work!
			// type.nonGenericTypeName = Objects.requireNonNull(deducePath.getType(i -1)).nonGenericTypeName;
		}
...
//			assert attached != null; // TODO will fail when empty variable expression
		if (attached != null && attached.getType() == OS_Type.Type.USER) {
			implement_construct_type(vte, attached, null, vte.getTypeTableEntry().getGenType());
		} else {
			final OS_Type ty2 = vte.getTypeTableEntry().getGenType().getTypeName();
			assert ty2 != null;
			implement_construct_type(vte, ty2, null, vte.getTypeTableEntry().getGenType());
		}
	}
