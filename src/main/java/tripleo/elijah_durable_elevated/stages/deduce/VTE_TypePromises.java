...
//							final BaseFunctionDef fd = gf.getFD();
					...
					if (pte.getFunctionInvocation() == null) {
						...
						tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_2("175 pte.fi is not null");
					aIntegerIA.gf.addDependentFunction(pte.getFunctionInvocation()); // TODO is this needed (here)?
				} else {
					int y = 3;
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_2("3074");
				}
			}
		});
	}
		...
		aVariableTableEntry.typePromise().then(new DoneCallback<GenType>() {
			...
			public void onDone(@NotNull GenType result) {
				...
				if (result.getResolved().getClassOf() != fd.getParent()) {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("EVTE-159 violation (likely String vs ConstString)");
				}

				final GenType genType1 = aVariableTableEntry.getTypeTableEntry().getGenType();
	...
	static void resolved_element_pte_VariableStatement2(final Constructable co,
		...
		aVariableTableEntry.typePromise().then(new DoneCallback<GenType>() {
			...
			public void onDone(@NotNull GenType result) {
				if (result.getResolved().getClassOf() != fd.getParent()) {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("** Failed assertion");
				}

				@NotNull
				ProcTableListener.E_Is_FunctionDef e_Is_FunctionDef = aProcTableListener.new E_Is_FunctionDef(pte, fd,
						fd.getParent()).invoke(aVariableTableEntry.getTypeTableEntry().getGenType().getNonGenericTypeName());
				@Nullable
				FunctionInvocation fi = e_Is_FunctionDef.getFi();
				GenType genType = e_Is_FunctionDef.getGenType();
