...
//				return deduceExpression(vs.initialValue(), ctx); // infinite recursion
	...
	private static @Nullable GenType deduceProcedureCall(final @NotNull ProcedureCallExpression pce,
		...
		tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("979 During deduceProcedureCall " + pce);
		@Nullable
		...
		if (!finished) {
			if (best != null) {
				...
				if (best instanceof ClassStatement) {
					...
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("992 " + best.getClass().getName());
