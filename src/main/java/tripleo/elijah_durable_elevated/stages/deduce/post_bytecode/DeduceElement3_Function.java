	...
	public GenType resolve_function_return_type_int(final @NotNull ErrSink errSink) {
		...
		if (vte_index != null) {
			...
			if (vte.getTypeTableEntry().getAttached() != null) {
				vte.resolveType(vte.getTypeTableEntry().getGenType()); // TODO doesn't fit pattern of returning and then setting
				return vte.getTypeTableEntry().getGenType();
			} else {
				@NotNull
