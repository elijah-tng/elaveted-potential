...
import tripleo.elijah_durable_elevated.stages.deduce.post_bytecode.DED.*;
...
public class DeduceElement3_IdentTableEntry extends DefaultStateful implements IDeduceElement3 {
	...
	private final DeduceElement3_Type _type = new DeduceElement3_Type() {
		...
		public GenType genType() {
			return typeTableEntry().getGenType();
		}

		//@Override
		public
		//@NotNull
		Operation2<GenType> resolved(final Context ectx) {
			try {
				if (_resolved == null) {
	...
	public void do_assign_normal_ident_deferred_VariableStatement(final VariableStatementImpl vs) {
		var dt2              = deduceTypes2;
		var aIdentTableEntry = principal;

		final DeduceElementWrapper parent = new DeduceElementWrapper(Objects.requireNonNull(vs.getParent()).getParent());
		final DeferredMember       dm     = parent.deferred_member(new do_assign_normal_ident_deferred__DT_ResolveObserver.X_S(generatedFunction).get(), vs, aIdentTableEntry, dt2);
		...
		final GenType genType = dt2._inj().new_GenTypeImpl();
	...
	public void assign_type_to_idte(final @NotNull Context aFunctionContext, final @NotNull Context aContext) {
		new ExitGetType().assign_type_to_idte(principal,
											  generatedFunction,
											  aFunctionContext,
											  aContext,
											  deduceTypes2,
	...
	public void dan(final @NotNull BaseEvaFunction generatedFunction, final @NotNull Instruction instruction,
		...
		if (idte.type == null) {
			@NotNull final Resolve_Ident_IA.DeduceElementIdent dei = this.principal.getDeduceElement();
			aDeduceTypes2.resolveIdentIA_(aContext, dei, generatedFunction, new FoundElement(aDeduceTypes2.phase) {
	...
	public @Nullable EvaNode getResolved() {
		...
		if (_type.typeTableEntry().getGenType().getTypeName() == null) {
		...
		if (principal.getResolvedElement() instanceof ClassStatement) {
			...
			final GenType gt1 = _type.typeTableEntry().getGenType();
			// gt1.typeName
		...
		EDL_ICompilation cc = this.deduceTypes2._phase()._compilation();
	...
	public void sneakResolve() {
		...
		if (el instanceof IdentExpression ie) {
			if (ie.getText().equals(ident.getText())) {
				final DE3_ITE_Holder de3_ite_holder = this.asElementHolder();
	...
	private void __sneak_el_null__bl1_not_null__possible_type_part(final @NotNull VariableTableEntry vte_bl1,
		...
		for (TypeTableEntry potentialType : vte_bl1.potentialTypes()) {
			var y = potentialType.getTableEntry();
			if (y instanceof ProcTableEntry pte) {
				...
				if (pte.getStatus() == BaseTableEntry.Status.KNOWN) {
					...
					// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("322 " + ci);
					// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("323 " + fi);

					if (dt2 != null) {
						var pt = dt2._inj().new_DR_PossibleTypeCI(ci, fi);
						b.addPossibleType(pt);
					} else {
						tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4("460460 Can't create PossibleType because dt2 == null");
					}
								...
								if (retvtety.getAttached() != null) {
									...
									if (resl != null) {
										...
										if (dt2 != null) {
											...
											tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4("460507 Can't create PossibleType because dt2 == null");
										}
									} else {
										// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("364 " + principal.getIdent().getText());
									}
								}
							}
						}
	...
	public do_assign_normal_ident_deferred__DT_ResolveObserver
	create_do_assign_normal_ident_deferred__DT_ResolveObserver(
			final @NotNull IdentTableEntry aIdentTableEntry,
			final @NotNull BaseEvaFunction aBaseEvaFunction) {
		assert aIdentTableEntry == principal;
		assert aBaseEvaFunction == this.generatedFunction;
		return new do_assign_normal_ident_deferred__DT_ResolveObserver(aIdentTableEntry, aBaseEvaFunction);
	}

	public DE3_ITE_Holder asElementHolder() {
		return deduceTypes2._inj().new_DE3_ITE_Holder(getPrincipal(), this);
	}
					...
					if (backlink instanceof final ProcTableEntry procTableEntry) {
						procTableEntry.typeResolvePromise().done((final @NotNull GenType result) -> {
							...
							for (EvaContainer.VarTableEntry entry : ((EvaContainerNC) result.getNode()).varTable) {
								if (!entry.isResolved()) {
									tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("629 entry not resolved " + entry.nameToken);
								}
							}
						});
					}
	...
	public class DE3_EH_GroundedVariableStatement implements IElementHolder {
		...
		public void commitGenTypeActions() {
			...
			if (genType == null) {
				genType = principal.type.getGenType();
			}

			for (setup_GenType_Action action : actions) {
				action.run(principal.type.getGenType(), arena);
			}
		}
	...
	public class do_assign_normal_ident_deferred__DT_ResolveObserver implements DT_ResolveObserver {
		...
		@Override
		public void onElement(@NotNull OS_Element best) {
			if (best != null) {
				...
				deduceTypes2._LOG().err("242 Bad lookup" + identTableEntry.getIdent().getText());
			}
		}

		public void do_assign_normal_ident_deferred_VariableStatement(final @NotNull BaseEvaFunction generatedFunction,
																	  final @NotNull IdentTableEntry aIdentTableEntry,
																	  final @NotNull VariableStatementImpl vs) {
			@NotNull final DeduceElement3_IdentTableEntry de3_ite = aIdentTableEntry.getDeduceElement3(deduceTypes2, generatedFunction);
			de3_ite.do_assign_normal_ident_deferred_VariableStatement(vs);
		}
		...
		static class X_S implements Supplier<IInvocation> {
