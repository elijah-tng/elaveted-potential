...
public class DeduceElement3_VarTableEntry implements IDeduceElement3 {
	...
	private static void noteNonsense(int code, String message) {
		tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_2(String.format("%d %s%n", code, message));
	}

	private static void noteNonsenseErr(int code, String message) {
		tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err2(String.format("** [noteNonsenseErr] %d %s%n", code, message));
	}
	...
	private void __one_potential(final @NotNull DeducePhase aDeducePhase,
		...
		if (!potentialType.isResolved()) {
			...
			// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4((__passthru));
			...
			if (attachedType == OS_Type.Type.USER_CLASS) {
				...
				@NotNull
				GenerateFunctions gf = aDeducePhase.getGeneratePhase()
						.getGenerateFunctions(xci.getKlass().getContext().module());
				WlGenerateClass wgc = dt2._inj().new_WlGenerateClass(gf, xci, aDeducePhase.getGeneratedClasses(),
