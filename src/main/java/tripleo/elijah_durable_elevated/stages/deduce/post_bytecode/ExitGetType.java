...
public class ExitGetType implements State {
	...
	public void assign_type_to_idte(@NotNull final IdentTableEntry ite,
		...
		if (!ite.hasResolvedElement()) {
			...
			final var dei = ident_a.getEntry().getDeduceElement();
			dt2.resolveIdentIA_(aContext, dei, generatedFunction, new FoundElement(phase) {

				final String path = dei.getIdentIAPathNormal(generatedFunction);
				...
				private void __foundElement_hasIdteType() {
					...
					case FUNCTION -> {
						...
						if (ite.type.getTableEntry() instanceof final @NotNull ProcTableEntry pte) {

						} else if (ite.type.getTableEntry() instanceof final @NotNull IdentTableEntry identTableEntry) {
				...
				public void foundElement(final @NotNull OS_Element x) {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("590-590 " + x);
				...
				private void use_user_class(@NotNull final OS_Type aType, @NotNull final IdentTableEntry aEntry) {
					...
					if (aEntry.constructable_pte != null) {
						final int yyy = 3;
						tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println2("use_user_class: " + cs);
					}
				}
			});
		}
