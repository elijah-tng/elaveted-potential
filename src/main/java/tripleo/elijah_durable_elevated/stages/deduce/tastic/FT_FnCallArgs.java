...
public class FT_FnCallArgs implements ITastic {
	...
	public void do_assign_call(final @NotNull BaseEvaFunction generatedFunction,
		...
		if (pte.expression_num instanceof @NotNull final IdentIA identIA) {
			...
			// 08/13 out
			// 10/14 in
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("--------------------- 158" + (generatedFunction._getIdentIAResolvable(identIA).getNormalPath(generatedFunction, identIA)));
	...
	public void do_assign_call(final @NotNull BaseEvaFunction generatedFunction,
		...
		for (final @NotNull TypeTableEntry tte : pte.getArgs()) {
			LOG.info("771 " + tte);
			final IExpression e = tte.get__debug_expression();
			if (e == null)
				continue;
			switch (e.getKind()) {
			case NUMERIC -> {
