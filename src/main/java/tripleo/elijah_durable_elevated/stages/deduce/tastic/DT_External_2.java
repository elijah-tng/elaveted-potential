...
public class DT_External_2 implements DT_External {
	...
	private /* static */ void __make2_1__hasFunctionInvocation(final @NotNull ProcTableEntry pte,
			...
			if (fi.pte == null) {
				tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("******************************* Unexpected error");
				return;
			}
	...
	private void onResolvedElement(OS_Element el) {
		if (el instanceof FunctionDef fd) {
			...
			target_p2.then(target0 -> {
				...
				if (target0 == null) {
					@NotNull Compilation c = el.getContext().module().getCompilation();
					if (c.reports().outputOn(Outs.OutDeduce_220))
						tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("542542  ");
					return;
				}
