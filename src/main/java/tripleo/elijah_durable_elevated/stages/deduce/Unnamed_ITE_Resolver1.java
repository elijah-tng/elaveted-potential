			...
			if (vte.getStatus() == BaseTableEntry.Status.KNOWN && vte.getTypeTableEntry().getAttached() != null
				...
				try {
					if (ty.getType() == OS_Type.Type.USER) {
						...
						OS_Element ele;
						if (vte.getTypeTableEntry().getGenType().getResolved() == null) {
							if (ty2.getResolved().getType() == OS_Type.Type.USER_CLASS) {
								vte.getTypeTableEntry().getGenType().copy(ty2);
							}
						}
			...
			if (ite_type != null) {
				...
				try {
					if (ty.getType() == OS_Type.Type.USER) {
						...
						GenType ty2 = dt2.resolve_type(ty, ty.getTypeName().getContext());
						OS_Element ele;
						if (ite_type.getGenType().getResolved() == null) {
							if (ty2.getResolved().getType() == OS_Type.Type.USER_CLASS) {
								ite_type.getGenType().copy(ty2);
							}
						}
						ele = ty2.getResolved().getElement();
					...
					if (ty2 != null) {
						...
						if (/* !ty.getTypeName().isNull() && */ !ty2.isNull()) {
							...
							if (!ty.getTypeName().isNull()) {
								...
								if (gp != null) {
									if (gp.size() > 0 && ite.type.getGenType().getNonGenericTypeName() == null) {
										skip = true;
									}
								}
							}
							if (!skip)
								ite.type.getGenType().genCIForGenType2(dt2);
						}
					}
				} catch (ResolveError aResolveError) {
					dt2._errSink().reportDiagnostic(aResolveError);
...
//							ite.setStatus(BaseTableEntry.Status.KNOWN, best);
				...
				final TypeName                  typeName = vte.getTypeTableEntry().getGenType().getNonGenericTypeName();
