...
public class DeducePath {
	...
	class MemberContext extends __Abstract_Context {
...
//				return null;
	...
	private final Eventual<@NotNull GenType>  [] typesP;

	@Contract(pure = true)
	public DeducePath(IdentTableEntry aIdentTableEntry, @NotNull List<InstructionArgument> aProvided) {
		final int size = aProvided.size();
		assert size > 0;
		...
		typesP   = new Eventual[size];
		for(int i = 0; i < size; i++) {
			typesP[i] = new Eventual<>();
		}
	}
...
//			identTableEntry.elementPromise((x)->{}, null);
	...
	public @Nullable GenType getType(int aIndex) {
		if (!(typesP[aIndex].isPending())) {
			return EventualExtract.of(typesP[aIndex]);
		//} else {
		//	return "undetermined";
		}
...

		{
			...
			if (ia2 instanceof IntegerIA) {
				...
				@Nullable  GenType gt3;
				gt3 = vte.getTypeTableEntry().getGenType();
				typesP[aIndex].resolve(gt3);
			} else if (ia2 instanceof IdentIA) {
				@NotNull IdentTableEntry identTableEntry = ((IdentIA) ia2).getEntry();
				if (identTableEntry.type != null) {
					@Nullable  GenType gt2;
