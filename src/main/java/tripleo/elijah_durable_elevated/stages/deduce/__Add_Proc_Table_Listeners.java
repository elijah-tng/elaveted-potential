...
public class __Add_Proc_Table_Listeners {
	class SL implements BaseTableEntry.StatusListener {
		...
		public void onChange(@NotNull IElementHolder eh, BaseTableEntry.Status newStatus) {
			...
			case CLASS -> {
				if (idte.type != null && idte.type.getGenType() != null) {
					assert idte.type.getGenType().getResolved() != null;
					generatedFunction.addDependentType(idte.type.getGenType());
				} else {
					@NotNull
			...
			case FUNCTION -> {
				...
				if (identIA2 != null) {
					...
					if (procTableEntry == pte)
						tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("940 procTableEntry == pte");
