...
import tripleo.elijah_durable_elevated.lang.impl.LangGlobals;
import tripleo.elijah_durable_elevated.stages.deduce.nextgen.DR_Ident;
import tripleo.elijah_durable_elevated.stages.deduce.post_bytecode.DG_ClassStatement;
...
public class ProcTableListener implements BaseTableEntry.StatusListener {
	...
	public void onChange(final IElementHolder eh, final BaseTableEntry.Status newStatus) {
		...
		if (newStatus != BaseTableEntry.Status.UNKNOWN) { // means eh is null
			...
			if (expressionNum instanceof IdentIA identIA) {
				...
				tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("*************************** i still refuse");
			}
		}
	}
	...
	private static void resolved_element_pte_ClassStatement_EvaClass(final EvaClass result,
		...
		// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("828282 "+((ClassStatement) e).name());
		if ((e.name()).equals("Foo")) {
			tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_4("828282 Foo found");
		}
	...
	void finish(@Nullable Constructable co, @Nullable AbstractDependencyTracker depTracker,
		...
		if (depTracker != null) {
			if (aGenType == null)
				tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("247 genType is null");
		...
		if (parent instanceof IdentExpression) {
			...
			if (fd == parent) {
				...
				if (x.getAttached() == null && x.getTableEntry() == null) {
					String text = ((IdentExpression) x.get__debug_expression()).getText();
					@Nullable
					InstructionArgument vte_ia = generatedFunction.vte_lookup(text);
					if (vte_ia != null) {
						GenType gt = ((IntegerIA) vte_ia).getEntry().getTypeTableEntry().getGenType();
						...
						if (parent instanceof ClassStatement) {
							...
							tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_2("NOTE ineresting in genericA/__preinc__");
						}
					}
				}
			}
