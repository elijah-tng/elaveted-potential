...
//			assert pte != null;
				...
				if (bl instanceof final @NotNull IntegerIA integerIA) {
					...
					if (ite.type.getGenType().getCi() != null)
						...
						final Operation<ClassInvocation> oi = ClassInvocationMake.withGenericPart(
								attached1.getClassOf(), null, (NormalTypeName) tte.getGenType().getNonGenericTypeName(),
								deduceTypes2);
	...
	private void ia2_IntegerIA_null_attached(@NotNull Context ctx, @NotNull List<TypeTableEntry> pot) {
		try {
			...
			TableEntryIV xx = pot.get(0).getTableEntry();
			if (xx != null) {
				...
				final IExpression debugExpression = pot.get(0).get__debug_expression();
				Aurum xxxxxx=deduceTypes2.onResolvedExpression(debugExpression, new DoneCallback<FluffyExpression>() {
					@Override
					public void onDone(final FluffyExpression result) {
						assert false;
					}
				});
				xxxxxx.resolveNow();
				...
				if (debugExpression instanceof ProcedureCallExpression pce_expression) {
					final DeduceUtils.MatchFunctionArgs matchFunctionArgs = new DeduceUtils.MatchFunctionArgs(pce_expression);
					@Nullable OS_Element                best              = lrl.chooseBest(Helpers.List_of(matchFunctionArgs));
	...
	private void ia2_IntegerIA_potentialTypes_equals_1(@NotNull VariableTableEntry aVte, String aText) {
		...
		TableEntryIV te = pot.get(0).getTableEntry();
	...
	public void resolveIdentIA2_(final @NotNull Context ctx, final @Nullable IdentIA identIA,
		...
		if (s == null)
			s = StaticEva._getIdentIAPathList(identIA);
