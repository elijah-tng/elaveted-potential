...
class Resolve_Variable_Table_Entry {
	...
	private void action_ARG(@NotNull VariableTableEntry vte) {
		...
		if (attached != null) {
			final GenType genType = tte.getGenType();
				...
				try {
					if (attached.getTypeName() instanceof RegularTypeName rtn) {
						if (rtn.getGenericPart() != null) {
							...
							// 08/13 tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("===== future 8181");
							return;
						}
					}
	...
	private void action_VAR(@NotNull VariableTableEntry vte) {
		if (vte.getTypeTableEntry().getAttached() == null && vte.getPotentialTypes().size() == 1) {
			...
			if (pot.getAttached() instanceof OS_FuncExprType) {
				action_VAR_potsize_1_and_FuncExprType(vte, (OS_FuncExprType) pot.getAttached(), pot.getGenType(),
													  pot.get__debug_expression());
...
//			tripleo.elijah.util.Stupidity.println_out_2("1630 "+mod_ns.getItems()); // element 0 is ctor$0
			...
			@NotNull
			GenerateFunctions gen = phase.getGeneratePhase().getGenerateFunctions(mod1);
			@NotNull
			...
			wl.addJob(deduceTypes2._inj().new_WlGenerateNamespace(gen, modi, phase.getGeneratedClasses(),
	...
	private void action_VAR_potsize_1_other(@NotNull VariableTableEntry vte, @NotNull TypeTableEntry aPot) {
		try {
			if (aPot.getTableEntry() instanceof final @NotNull ProcTableEntry pte1) {
				...
				if (e != null) {
					...
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("118118 Making external for " + debugExpression);
				}
