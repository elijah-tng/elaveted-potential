...
import tripleo.elijah_durable_elevated.nextgen.*;
import tripleo.elijah_durable_elevated.nextgen.diagnostic.*;
...
public class DeducePhase extends _RegistrationTarget implements ReactiveDimension {
	...
	private final @NotNull GeneratedClasses                             generatedClasses;
	private final @NotNull GeneratePhase                                generatePhase;
	@NotNull
	private final          List<IFunctionMapHook>                       functionMapHooks;
	private final          Multimap<OS_Module, Consumer<DeduceTypes2>>  iWantModules;
	@Getter
	...
	private final          List<DeferredMemberFunction>                 deferredMemberFunctions;
	private final          List<FoundElement>                           foundElements;
	private final          Multimap<FunctionDef, EvaFunction>           functionMap;
	private final          Map<IdentTableEntry, OnType>                 idte_type_callbacks;
	...
	public DeducePhase(final @NotNull ICompilationAccess aca, final @NotNull EDL_IPipelineAccess pa0,
					   final @NotNull EDL_PipelineLogic aPipelineLogic) {
		// given
		...
		functionMap             = ArrayListMultimap.create();
		...
		deferredMemberFunctions = _inj().new_ArrayList__DeferredaMemberFunction();
		foundElements           = _inj().new_ArrayList__FoundElement();
		...
		idte_type_callbacks     = _inj().new_HashMap__IdentTableEntry();
	...
	public EDL_ICompilation _compilation() {
...
//					return aClassInvocation;
	...
	public void finish() {
		...
		for (Pair<BaseEvaFunction, DR_Item> pair : drs.iterator()) {
			...
			// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("611a " + ef);
			// tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("611b " + dr);
			...
			if (dr instanceof DR_ProcCall drpc) {
				...
				SimplePrintLoggerToRemoveSoon.println_err_4(String.format("***** 623623 -- %s %b", drid.name(),
																		  drid.isResolved()));

				for (DR_Ident.Understanding understanding : drid.u) {
					SimplePrintLoggerToRemoveSoon.println_err_4(String.format("**** 623626 -- %s",
...
//								}
			...
			} else if (deferredMember.getParent().element() instanceof final ClassStatement parent) {
				// TODO do something
					...
					for (EvaContainer.VarTableEntry gc_vte : vt) {
						if (gc_vte.nameToken.getText().equals(name)) {
							...
							if (!connectionPairs.isEmpty()) {
								final GenType ty = connectionPairs.get(0).vte.getTypeTableEntry().getGenType();
...
//		LOG.err("272 forFunction\n\t"+fi.getFunction()+"\n\t"+fi.pte);
	...
	public @NotNull List<IFunctionMapHook> getFunctionMapHooks() {
		return functionMapHooks;
	}

	public Multimap<OS_Module, Consumer<DeduceTypes2>> getiWantModules() {
		return iWantModules;
	}

	public IPipelineAccess getPa() {
		return pa;
	}

	public void setPa(EDL_IPipelineAccess aPa) {
		pa = aPa;
	}
	...
	public class DeducePhaseInjector {
		...
		public DeduceTypes2 new_DeduceTypes2(final OS_Module aM, final DeducePhase aDeducePhase,
											 final EDL_ElLog.Verbosity aVerbosity) {
		...
		public ElLog new_ElLog(final String aS, final EDL_ElLog.Verbosity aVerbosity, final String aDeducePhase) {
			return new EDL_ElLog(aS, aVerbosity, aDeducePhase);
		}
	...
	/* static */ class DeferredMemberFunctionParentIsClassStatement {
		...
		<T extends EvaNode> void defaultAction(final T result) {
			...
			if (p instanceof OS_SpecialVariable) {
				final @NotNull OS_SpecialVariable specialVariable = (OS_SpecialVariable) p;
