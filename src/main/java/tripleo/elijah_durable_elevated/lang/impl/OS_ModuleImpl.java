...
public class OS_ModuleImpl implements OS_Element, OS_Container, tripleo.elijah.lang.i.OS_Module {
	...
	private EDL_ICompilation     parent;
	...
	private void find_multiple_items(Complaint c) {
		// FIXME 10/19
		final EDL_ICompilation cc = getCompilation();
	...
	@Override
	public void finish() {
//		parent.put_module(fileName, this);
	}
...
//		}
	...
	public void postConstruct() {
		...
		if (false) {
			getCompilation().getFluffy().module(this).find_all_entry_points();
		} else {
...
//							tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("159159 " + classItemParent.getClass().getName());
						...
						tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_out_2("243 " + entryPoints + " " + _fileName);
//					break; // allow for "extend" class
	...
	public void serializeTo(final @NotNull SmallWriter sw) {
		...
		// public @NotNull Attached attached = new AttachedImpl();
		// private final Stack<Qualident> packageNames_q = new Stack<Qualident>();
		// public @NotNull List<EntryPoint> entryPoints = new ArrayList<EntryPoint>();
		// private IndexingStatement indexingStatement;
		// private LibraryStatementPart lsp;
	...
	public void setParent(@NotNull final Compilation parent) {
		this.parent = (EDL_ICompilation) parent;
	}
