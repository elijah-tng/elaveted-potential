...
public class ElijjahParser extends antlr.LLkParser implements ElijjahTokenTypes {
	...
	public final AliasStatement aliasStatement(OS_Element cont) throws RecognitionException, TokenStreamException {
		...
		pc = pcon.new_AliasStatement(cont);
	...
	public final @Nullable ClassHeader class_header(List<AnnotationClause> as)
		...
		try { // for error handling
			...
			match(LITERAL_class);
			if (inputState.guessing == 0) {
				ch = pcon.new_ClassHeader(extends_, as);
			}
			class_modifier(ch);
			class_name = ident();
			if (inputState.guessing == 0) {
	...
	public final @Nullable ClassStatement classStatement(OS_Element parent, Context cctx, List<AnnotationClause> as)
		...
		try { // for error handling
			ch = class_header(as);
			if (inputState.guessing == 0) {
				cls = pcon.new_ClassStatement(parent, cctx);
				cls.setHeader(ch);
			}
			match(LCURLY);
			if (inputState.guessing == 0) {
	...
	public final @Nullable IExpression constantValue() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			case CHAR_LITERAL: {
				...
				match(CHAR_LITERAL);
				if (inputState.guessing == 0) {
					e = pcon.new_CharLitExpression(c);
				}
				break;
			}
			case NUM_INT: {
				...
				match(NUM_INT);
				if (inputState.guessing == 0) {
					e = pcon.new_NumericExpression(n);
				}
				break;
			}
			case NUM_FLOAT: {
	...
	public final @Nullable DefFunctionDef def_function_definition(OS_Element parent, Context ctx,
		...
		try { // for error handling
			...
			fh = function_header();
			if (inputState.guessing == 0) {
				fd = pcon.new_DefFunctionDef(parent, ctx);
				cur = fd.getContext();
			}
			fb = expression();
			{
	...
	public final @Nullable IExpression dot_expression_or_procedure_call(IExpression e1)
		...
		try { // for error handling
			e = ident();
			if (inputState.guessing == 0) {
				ee = pcon.new_DotExpression(e1, e);
			}
			{
				if ((LA(1) == LPAREN) && (_tokenSet_59.member(LA(2)))) {
					lp2 = LT(1);
					...
					if (inputState.guessing == 0) {
						ProcedureCallExpression pce = pcon.new_ProcedureCallExpression();
						pce.identifier(ee);
						pce.setArgs(el);
						ee = pce;
					}
	...
	public final FunctionBody function_body_mandatory(OS_Element parent)
		...
		fb = pcon.new_FunctionBody();
		Scope3 sc = pcon.new_Scope3(parent);
	...
	public final GenericTypeName genericTypeName2() throws RecognitionException, TokenStreamException {
		...
		tn = pcon.new_GenericTypeName(cur);
		TypeName tn2 = null;
	...
	public final @Nullable IdentExpression ident() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			match(IDENT);
			if (inputState.guessing == 0) {
				id = pcon.new_IdentExpression(r1, inputState.getFilename(), cur);
			}
		} catch (RecognitionException ex) {
			if (inputState.guessing == 0) {
				reportError(ex);
	...
	public final void importPart2(QualifiedImportStatement cr) throws RecognitionException, TokenStreamException {
		...
		Qualident q3;
		IdentList il = pcon.new_IdentList();
	...
	public final @Nullable IndexingStatement indexingStatement() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			if (inputState.guessing == 0) {
				idx = pcon.new_IndexingStatement(out.module());
			}
			{
				_loop7: do {
					if ((LA(1) == IDENT)) {
						...
						el = expressionList();
						if (inputState.guessing == 0) {
							item = pcon.new_IndexingItem(i1, el);
							idx.add(item);
						}
					} else {
						break;
		...
		try { // for error handling
			match(LITERAL_invariant);
			{
				_loop173: do {
					if ((_tokenSet_18.member(LA(1)))) {
						if (inputState.guessing == 0) {
							isp = pcon.new_InvariantStatementPart(cr, i1);
						}
						{
	...
	public final @Nullable FunctionDef normal_function_definition(OS_Element parent, Context ctx,
		...
		try { // for error handling
			fh = function_header();
			if (inputState.guessing == 0) {
				fd = pcon.new_FunctionDef(parent, ctx);
				cur = fd.getContext();
			}
			fb = function_body(fd);
			if (inputState.guessing == 0) {
	...
	public final NormalTypeName normalTypeName2() throws RecognitionException, TokenStreamException {
		...
		tn = pcon.new_RegularTypeName(cur);
		...
		try { // for error handling
			ee = primaryExpression();
			{
				_loop225: do {
					if ((LA(1) == DOT) && (LA(2) == IDENT)) {
						...
						match(RBRACK);
						if (inputState.guessing == 0) {
							ee = pcon.new_GetItemExpression(ee, expr);
						...
						}
						{
							if ((LA(1) == BECOMES) && (_tokenSet_18.member(LA(2)))) {
								...
								expr = expression();
								if (inputState.guessing == 0) {
									ee = pcon.new_SetItemExpression((GetItemExpression) ee, expr);
								}
							} else if ((_tokenSet_7.member(LA(1))) && (_tokenSet_56.member(LA(2)))) {
							} else {
								throw new NoViableAltException(LT(1), getFilename());
						...
						if (inputState.guessing == 0) {
							ProcedureCallExpression pce = pcon.new_ProcedureCallExpression(ee, el);
							ee = pce;
						}
						match(RPAREN);
					} else {
			...
			}
			{
				if ((LA(1) == AS || LA(1) == CAST_TO) && (_tokenSet_61.member(LA(2)))) {
					if (inputState.guessing == 0) {
						tc = pcon.new_TypeCastExpression();
						ee = tc;
					}
					{
						switch (LA(1)) {
		...
		try { // for error handling
			...
			case LITERAL_namespace: {
				...
				}
				{
					switch (LA(1)) {
					case LITERAL_namespace: {
						namespaceStatement(pcon.new_NamespaceStatement(cont, cur), as);
						break;
					}
					case LITERAL_extend:
					case LITERAL_class: {
		...
		try { // for error handling
			ee = shiftExpression();
			{
				if ((_tokenSet_7.member(LA(1))) && (_tokenSet_56.member(LA(2)))) {
					...
					tn = typeName2();
					if (inputState.guessing == 0) {
						ee = pcon.new_TypeCheckExpression(ee, tn);
					}
				} else {
					throw new NoViableAltException(LT(1), getFilename());
				}
	...
	public final void syntacticBlockScope(OS_Element aParent) throws RecognitionException, TokenStreamException {
		...
		SyntacticBlock sb = pcon.new_SyntacticBlock(aParent);
	...
	public final TypeOfTypeName typeOfTypeName2() throws RecognitionException, TokenStreamException {
		...
		tn = pcon.new_TypeOfTypeName(cur);
	...
	public final @Nullable IExpression unaryExpression() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			case INC: {
				...
				ee = unaryExpression();
				if (inputState.guessing == 0) {
					ee = pcon.new_UnaryExpression(ExpressionKind.INCREMENT, ee);
				}
				break;
			}
			case DEC: {
				...
				ee = unaryExpression();
				if (inputState.guessing == 0) {
					ee = pcon.new_UnaryExpression(ExpressionKind.DECREMENT, ee);
				}
				break;
			}
			case MINUS: {
				...
				ee = unaryExpression();
				if (inputState.guessing == 0) {
					ee = pcon.new_UnaryExpression(ExpressionKind.NEG, ee);
				}
				break;
			}
			case PLUS: {
				...
				ee = unaryExpression();
				if (inputState.guessing == 0) {
					ee = pcon.new_UnaryExpression(ExpressionKind.POS, ee);
				}
				break;
			}
			case IDENT:
	...
	public final @Nullable IExpression unaryExpressionNotPlusMinus() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			case BNOT: {
				...
				ee = unaryExpression();
				if (inputState.guessing == 0) {
					ee = pcon.new_UnaryExpression(ExpressionKind.BNOT, ee);
				}
				break;
			}
			case LNOT: {
				...
				ee = unaryExpression();
				if (inputState.guessing == 0) {
					ee = pcon.new_UnaryExpression(ExpressionKind.LNOT, ee);
				}
				break;
			}
			case IDENT:
	...
	public final void withStatement(OS_Element aParent) throws RecognitionException, TokenStreamException {
		...
		WithStatement ws = pcon.new_WithStatement(aParent);
