...
class ResolvedNodes {
	...
	public void init(final DeducePhase.@NotNull GeneratedClasses c) {
		// 09/26 tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("2222 " + c);
	...
	public void part2() {
		resolved_nodes.stream()
				...
				.filter(coded -> coded.getCode() == 0).forEach(coded -> {
					tripleo.elijah_fluffy.util.SimplePrintLoggerToRemoveSoon.println_err_4("-*-*- __processResolvedNodes [NOT CODED] " + coded);
					coded.register(cr);
				});
	}

	public void part3(final @NotNull EDL_PipelineLogic pipelineLogic,
