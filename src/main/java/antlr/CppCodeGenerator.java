...
public class CppCodeGenerator extends CodeGenerator {
    ...
    public void printAction(Token t) {
        if (t != null) {
            genLineNo(t.getLine());
            printTabs();
            _printAction(processActionForSpecialSymbols(t.getText(), t.getLine(), null, null));
            genLineNo2();
        }
    ...
    public void printHeaderAction(String name) {
        ...
        if (a != null) {
            genLineNo(a.getLine());
            println(processActionForSpecialSymbols(a.getText(), a.getLine(), null, null));
            genLineNo2();
        }
    ...
    public void genLineNo(int line) {
        ...
        if (genHashLines) _println("#line " + line + " \"" + antlrTool.fileMinusPath(antlrTool.grammarFile) + "\"");
    ...
    public void genLineNo(GrammarElement el) {
        if (el != null) genLineNo(el.getLine());
    }

    /** Generate a #line or // line depending on options */
    public void genLineNo(Token t) {
        if (t != null) genLineNo(t.getLine());
    }
    ...
    protected void genAlt(Alternative alt, AlternativeBlock blk) {
        ...
        if (genAST) {
            if (blk instanceof RuleBlock) {
                ...
                else println(rblk.getRuleName() + "_AST = currentAST.root;");
                ...
                antlrTool.warning(
                        "Labeled subrules are not implemented", grammar.getFilename(), blk.getLine(), blk.getColumn());
            }
        }
    ...
    protected void genBitsets(Vector bitsetList, int maxVocabulary, String prefix) {
        ...
        for (int i = 0; i < bitsetList.size(); i++) {
            ...
            for (int j = 0; j < tm.getVocabulary().size(); j++) {
                if (p.member(j)) {
                    if ((grammar instanceof LexerGrammar)) {
                        ...
                        if ((0x20 <= j) && (j < 0x7F) && (j != '\\')) t += charFormatter.escapeChar(j, true) + " ";
            ...
            // BitSet object
            println("const " + namespaceAntlr + "BitSet " + prefix + getBitsetName(i) + "(" + getBitsetName(i)
                    + "_data_," + p.size() / 32 + ");");
        }
    }
    ...
    protected void genBlockInitAction(AlternativeBlock blk) {
        ...
        if (blk.initAction != null) {
            genLineNo(blk);
            printAction(processActionForSpecialSymbols(blk.initAction, blk.line, currentRule, null));
            genLineNo2();
        }
    }
    ...
    public void genBody(LexerGrammar g) throws IOException {
        ...
        if (grammar.debuggingOutput)
            println(": " + sup + "(new " + namespaceAntlr + "DebuggingInputBuffer(new " + namespaceAntlr
        ...
        println(grammar.getClassName() + "::" + grammar.getClassName() + "(const " + namespaceAntlr
    ...
    private String convertJavaToCppString(String lit, boolean isCharLiteral) {
        ...
        while (i < s.length()) {
            if (s.charAt(i) == '\\') {
                if (s.length() == i + 1)
                    antlrTool.error(
                            "Invalid escape in char literal: '" + lit + "' looking at '" + s.substring(i) + "'");
                ...
                switch (s.charAt(i + 1)) {
                    ...
                    case 'u':
                        ...
                        if (i + 5 < s.length()) {
                            ...
                            antlrTool.error("Invalid escape in char literal: '" + lit + "' looking at '"
                                    + s.substring(i) + "'");
                        break;
                    ...
                    default:
                        antlrTool.error(
                                "Unhandled escape in char literal: '" + lit + "' looking at '" + s.substring(i) + "'");
                        val = 0;
                }
            } else val = s.charAt(i++);
            ...
            if (grammar instanceof LexerGrammar) {
                ...
                {
                    String offender;
                    if ((0x20 <= val) && (val < 0x7F)) offender = charFormatter.escapeChar(val, true);
                    else offender = "0x" + Integer.toString(val, 16);

                    antlrTool.error("Character out of range in " + (isCharLiteral ? "char literal" : "string constant")
                            + ": '" + s + "'");
        ...
        if (!isCharLiteral) ret = prefix + "\"" + ret + "\"";
        return ret;
    }
                ...
                if (ts.getASTNodeType() != null) {
                    ...
                    if (type == null) astTypes.setElementAt(ts.getASTNodeType(), ts.getTokenType());
                        ...
                        if (!ts.getASTNodeType().equals(type)) {
                            antlrTool.warning(
                                    "Token " + tok + " taking most specific AST type", grammar.getFilename(), 1, 1);
                            antlrTool.warning(
                                    "  using " + type + " ignoring " + ts.getASTNodeType(),
                                    grammar.getFilename(),
                                    1,
                                    1);
                        }
    ...
    public CppBlockFinishingInfo genCommonBlock(AlternativeBlock blk, boolean noTestForSingle) {
        ...
        for (int i = 0; i < blk.getAlternatives().size(); i++) {
            ...
            if (suitableForCaseExpression(a)) nLL1++;
        }
            ...
            for (int i = 0; i < blk.alternatives.size(); i++) {
                ...
                if (alt.cache[1].fset.degree() > caseSizeThreshold && suitableForCaseExpression(alt)) {
                    ...
                    if (alt.semPred != null) {
                        ...
                        ActionTransInfo tInfo = new ActionTransInfo();
                        String actionStr = processActionForSpecialSymbols(alt.semPred, blk.line, currentRule, tInfo);
            ...
            if (genAST) {
                switch (el.getAutoGenType()) {
                    case GrammarElement.AUTO_GEN_NONE:
                        if (usingCustomAST
                                || (el instanceof GrammarAtom && ((GrammarAtom) el).getASTNodeType() != null))
    ...
    public void gen(CharRangeElement r) {
        ...
        if (!(grammar instanceof LexerGrammar)) antlrTool.error("cannot ref character range in grammar: " + r);
        ...
        boolean save =
                (grammar instanceof LexerGrammar && (!saveText || r.getAutoGenType() == GrammarElement.AUTO_GEN_BANG));
        if (save) println("_saveIndex=text.length();");

        println("matchRange(" + convertJavaToCppString(r.beginText, true) + ","
                + convertJavaToCppString(r.endText, true) + ");");

        if (save) println("text.erase(_saveIndex);");
    }
    ...
    public void gen(OneOrMoreBlock blk) {
        ...
        genBlockFinish(howToFinish, "if ( " + cnt + ">=1 ) { goto " + label + "; } else {" + throwNoViable + "}");
    ...
    public void gen(RuleRefElement rr) {
        ...
        if (rs == null || !rs.isDefined()) {
            // Is this redundant???
            antlrTool.error(
                    "Rule '" + rr.targetRule + "' is not defined", grammar.getFilename(), rr.getLine(), rr.getColumn());
            return;
        }
    ...
    public void genInclude(LexerGrammar g) throws IOException {
        ...
        if (grammar.classMemberAction != null) {
            ...
            print(processActionForSpecialSymbols(
                    grammar.classMemberAction.getText(), grammar.classMemberAction.getLine(), currentRule, null));
            genLineNo2();
        }
    ...
    protected void genASTDeclaration(AlternativeElement el) {
        ...
        if (t.root.getLabel() != null) {
            println(t.root.getLabel() + " = (_t == " + labeledElementType + "(ASTNULL)) ? " + labeledElementASTInit
                    + " : _t;");
        }
    ...
    public void gen(WildcardElement wc) {
        ...
        if (grammar instanceof TreeWalkerGrammar) {
            println("if ( _t == " + labeledElementASTInit + " ) throw " + namespaceAntlr
    ...
    public void genNextToken() {
        ...
        if (!hasPublicRules) {
            println("");
            println(namespaceAntlr + "RefToken " + grammar.getClassName() + "::nextToken() { return " + namespaceAntlr
                    + "RefToken(new " + namespaceAntlr + "CommonToken(" + namespaceAntlr
                    + "Token::EOF_TYPE, \"\")); }");
            println("");
            return;
        }
        ...
        String errFinish = "if (LA(1)==EOF_CHAR)" + newline + "\t\t\t\t{"
                + newline + "\t\t\t\t\tuponEOF();" + newline + "\t\t\t\t\t_returnToken = makeToken("
                + namespaceAntlr + "Token::EOF_TYPE);" + newline
                + "\t\t\t\t}";
        errFinish += newline + "\t\t\t\t";
        if (((LexerGrammar) grammar).filterMode) {
            if (filterRule == null) {
                ...
                errFinish += "else {" + newline + "\t\t\t\t\tcommit();"
                        + newline + "\t\t\t\t\ttry {m"
                        + filterRule + "(false);}" + newline + "\t\t\t\t\tcatch("
                        + namespaceAntlr + "RecognitionException& e) {" + newline + "\t\t\t\t\t	// catastrophic failure"
                        + newline + "\t\t\t\t\t	reportError(e);"
                        + newline + "\t\t\t\t\t	consume();"
                        + newline + "\t\t\t\t\t}"
                        + newline + "\t\t\t\t\tgoto tryAgain;"
                        + newline + "\t\t\t\t}";
            }
        } else {
    ...
    public void genRule(RuleSymbol s, boolean startSymbol, int ruleNum, String prefix) {
            ...
            if (eqpos != -1) {
                ...
                while (cmpos != -1 && eqpos != -1) {
                    ...
                    if (cmpos != -1) {
                        ...
                        if (eqpos == -1) newarg = newarg + comma + oldarg;
                    }
                }
            } else newarg = oldarg;
        ...
        if (grammar instanceof TreeWalkerGrammar) {
            ...
            println(labeledElementASTType + " " + s.getId() + "_AST_in = (_t == " + labeledElementASTType
        ...
        if (grammar instanceof LexerGrammar) {
            println("if ( _createToken && _token==" + namespaceAntlr + "nullToken && _ttype!=" + namespaceAntlr
                    + "Token::SKIP ) {");
    ...
    private void GenRuleInvocation(RuleRefElement rr) {
        ...
        if (rr.args != null) {
            ...
            String args = processActionForSpecialSymbols(rr.args, rr.line, currentRule, tInfo);
    ...
    protected void genSemPredMap(String prefix) {
        ...
        tabs++;
        while (e.hasMoreElements()) println("\"" + e.nextElement() + "\",");
        println("0");
        tabs--;
        println("};");
    }
    ...
    public String getASTCreateString(GrammarAtom atom, String str) {
        if (atom != null && atom.getASTNodeType() != null) {
            ...
            if (type == null) astTypes.setElementAt(atom.getASTNodeType(), atom.getType());
            else {
                // give a warning over action taken if the types are unequal
                if (!atom.getASTNodeType().equals(type)) {
                    antlrTool.warning(
                            ...
                            atom.getColumn());
                    antlrTool.warning(
                            " from \"" + type + "\" to \"" + atom.getASTNodeType() + "\" sticking to \"" + type + "\"",
                            grammar.getFilename(),
                            atom.getLine(),
                            atom.getColumn());
                } else astTypes.setElementAt(atom.getASTNodeType(), atom.getType());
            }
            ...
            if (usingCustomAST
                    ...
                    && !(grammar.tokenManager.tokenDefined(str))
                    && !is_constructor) return "astFactory->create(" + namespaceAntlr + "RefAST(" + str + "))";
    ...
    private void setupGrammarParameters(Grammar g) {
        if (g instanceof ParserGrammar || g instanceof LexerGrammar || g instanceof TreeWalkerGrammar) {
            ...
            if (g.hasOption("namespaceAntlr")) {
                ...
                if (t != null) {
                    ...
                    if (ns != null) {
                        if (ns.length() > 2
                                && !ns.substring(ns.length() - 2, ns.length()).equals("::")) ns += "::";
                        namespaceAntlr = ns;
                    }
                }
            }
        ...
        if (g instanceof ParserGrammar) {
            ...
            commonLocalVars = "int _ttype; " + namespaceAntlr + "RefToken _token; " + namespaceStd
            ...
            if (g.hasOption("ASTLabelType")) {
                ...
                if (tsuffix != null) {
                    ...
                    if (suffix != null) {
                        ...
                        throwNoViable =
                                "throw " + namespaceAntlr + "NoViableAltException(" + namespaceAntlr + "RefAST(_t));";
                        lt1Value = "_t";
                    }
                }
            }
