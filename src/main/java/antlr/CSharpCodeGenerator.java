...
public class CSharpCodeGenerator extends CodeGenerator {
    ...
    public void gen(ActionElement action) {
        ...
        if (action.isSemPred) {
            ...
            if (tInfo.assignToRoot) {
                ...
                println("if ( (null != " + tInfo.refRuleRoot + ") && (null != " + tInfo.refRuleRoot
                        + ".getFirstChild()) )");
                tabs++;
    ...
    public void gen(CharRangeElement r) {
        ...
        if (flag) println("_saveIndex = text.Length;");

        println("matchRange(" + OctalToUnicode(r.beginText) + "," + OctalToUnicode(r.endText) + ");");

        if (flag) println("text.Length = _saveIndex;");
    }
    ...
    public void gen(OneOrMoreBlock blk) {
        ...
        genBlockFinish(
                howToFinish, "if (" + cnt + " >= 1) { goto " + label + "_breakloop; } else { " + throwNoViable + "; }");

        println(cnt + "++;");
        tabs--;
        if (blockNestingLevel-- == saveIndexCreateLevel) saveIndexCreateLevel = 0;
        println("}");
            ...
            if (genAST) {
                switch (rr.getAutoGenType()) {
                    case GrammarElement.AUTO_GEN_NONE:
                        if (usingCustomAST) println("astFactory.addASTChild(ref currentAST, (AST)returnAST);");
                        else println("astFactory.addASTChild(ref currentAST, returnAST);");
                        break;
    ...
    public String getASTCreateString(GrammarAtom atom, String astCtorArgs) {
        ...
        if (atom == null) return getASTCreateString(astCtorArgs);
        else {
            if (atom.getASTNodeType() != null) {
                ...
                if ((ts == null) || (ts.getASTNodeType() != atom.getASTNodeType()))
                    astCreateString = "(" + atom.getASTNodeType() + ") astFactory.create(" + astCtorArgs + ", \""
                            + atom.getASTNodeType() + "\")";
    ...
    protected String processActionForSpecialSymbols(
        ...
        if ((grammar.buildAST && actionStr.indexOf('#') != -1)
            ...
            antlr.actions.csharp.ActionLexer lexer =
                    new antlr.actions.csharp.ActionLexer(actionStr, currentRule, this, tInfo);
    ...
    public void genBody(ParserGrammar g) throws IOException {
        ...
        if (tsuffix != null) {
            String suffix = StringUtils.stripFrontBack(tsuffix.getText(), "\"", "\"");
            if (suffix != null) print("              , " + suffix); // must be an interface name for CSharp
        }
        println("{");
        tabs++;
     ...
     */
    public CSharpBlockFinishingInfo genCommonBlock(AlternativeBlock blk, boolean noTestForSingle) {
        int nIF = 0;
            ...
            if (genAST) {
                switch (el.getAutoGenType()) {
                    case GrammarElement.AUTO_GEN_NONE:
                        ...
                        else println("astFactory.addASTChild(ref currentAST, " + astName + ");");
                        break;
                    case GrammarElement.AUTO_GEN_CARET:
                        if (usingCustomAST
                                || ((el instanceof GrammarAtom) && (((GrammarAtom) el).getASTNodeType() != null)))
                            println("astFactory.makeASTRoot(ref currentAST, (AST)" + astName + ");");
                        else println("astFactory.makeASTRoot(ref currentAST, " + astName + ");");
                        break;
                    default:
                        break;
                }
