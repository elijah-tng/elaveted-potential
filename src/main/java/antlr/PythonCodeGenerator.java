...
public class PythonCodeGenerator extends CodeGenerator {
    ...
    protected void checkCurrentOutputStream() {
        try {
            if (currentOutput == null) throw new NullPointerException();
        } catch (Exception e) {
    ...
    public void gen(ActionElement action) {
        if (action.isSemPred) {
            ...
            if (tInfo.assignToRoot) {
                ...
                println("if (" + tInfo.refRuleRoot + " != None) and (" + tInfo.refRuleRoot
                        + ".getFirstChild() != None):");
                tabs++;
    ...
    public void gen(OneOrMoreBlock blk) {
        ...
        if (!blk.greedy
            ...
            if (!blk.greedy && blk.exitLookaheadDepth == LLkGrammarAnalyzer.NONDETERMINISTIC) {
                generateNonGreedyExitPath = true;
            }
        }
    ...
    public void gen(ParserGrammar g) throws IOException {
        ...
        if (grammar.buildAST) {
            ...
            if (labeledElementASTType != null) {
                println("self.astFactory.setASTNodeClass(" + labeledElementASTType + ")");
            }
        }
        genHeaderInit(grammar);
        println("");
            ...
            for (int i = 0; i < blk.alternatives.size(); i++) {
                ...
                if (grammar instanceof LexerGrammar) {
                    ...
                    if (effectiveDepth != altDepth) {
                        if (DEBUG_CODE_GENERATOR)
                            System.out.println(
                                    "ignoring alt because effectiveDepth!=altDepth" + effectiveDepth + "!=" + altDepth);
                        continue;
                    }
    ...
    public void genNextToken() {
        ...
        PythonBlockFinishingInfo howToFinish = genCommonBlock(nextTokenBlk, false);
        ...
        if (((LexerGrammar) grammar).filterMode) {
            ...
            if (filterRule == null) {
                ...
                errFinish += "self.filterdefault(self.LA(1), self.m" + filterRule + ", False)";
            }
        } else {
            /* non filter */
    ...
    public void genRule(RuleSymbol s, boolean startSymbol, int ruleNum) {
        ...
        if (rblk.returnAction != null) {
            if (rblk.returnAction.indexOf('=') >= 0) println(rblk.returnAction);
            else {
                // mx
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            if (s.getId().equals("mEOF")) println("_ttype = EOF_TYPE");
        ...
        if (grammar.debuggingOutput)
            if (grammar instanceof ParserGrammar) println("self.fireEnterRule(" + ruleNum + ", 0)");
            else if (grammar instanceof LexerGrammar) println("self.fireEnterRule(" + ruleNum + ", _ttype)");
        ...
        if (grammar.debuggingOutput || grammar.traceRules) {
            ...
            if (grammar.debuggingOutput)
                if (grammar instanceof ParserGrammar) println("self.fireExitRule(" + ruleNum + ", 0)");
                else if (grammar instanceof LexerGrammar) println("self.fireExitRule(" + ruleNum + ", _ttype)");
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            if (s != null) {
                ...
                if (ts != null && ts.getASTNodeType() != null) {
                    ...
                    println("self.tokenTypeToASTClassMap[" + ts.getTokenType() + "] = " + ts.getASTNodeType());
                }
            }
        }
        ...
        for (int i = Token.MIN_USER_TYPE; i < v.size(); i++) {
            ...
            if (s != null) {
                if (s.startsWith("\"")) {
                    ...
                    if (sl == null) antlrTool.panic("String literal " + s + " not in symbol table");
    ...
    public String getRangeExpression(int k, int[] elems) {
        ...
        return "(" + lookaheadString(k) + " >= " + getValueString(begin, true) + " and " + lookaheadString(k) + " <= "
    ...
    protected String processActionForSpecialSymbols(
        ...
        antlr.actions.python.ActionLexer lexer =
                new antlr.actions.python.ActionLexer(actionStr, currentRule, this, tInfo);
    ...
    protected String processActionCode(String actionStr, int line) {
        ...
        if (actionStr == null || isEmpty(actionStr)) return "";

        antlr.actions.python.CodeLexer lexer =
                new antlr.actions.python.CodeLexer(actionStr, grammar.getFilename(), line, antlrTool);
